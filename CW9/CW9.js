...
for (const simpson of simpsons) {
    ...
    member.style.width = `700px`;
    member.style.border = `3px solid black`;
    member.style.padding = `10px`;
    member.style.margin = `10px`;
    let member_name = document.createElement(`div`);
    member_name.textContent = `${simpson.name} - ${simpson.surname}`;
    let member_age = document.createElement(`div`);
    member_age.textContent = `Age - ${simpson.age} years`;
    let member_info = document.createElement(`div`);
    member_info.textContent = `${simpson.info}`;
    let member_image = document.createElement(`img`);
    member_image.src = simpson.photo;


    document.body.appendChild(member);
    member.appendChild(member_name);
    member.appendChild(member_age);
    member.appendChild(member_info);
    member.appendChild(member_image);
};
// - Є масив
let coursesArray = [
    {
...
let main_course = document.createElement(`div`)
main_course.classList.add(`main`)
document.body.appendChild(main_course)
let main_container = document.getElementsByClassName(`main`)[0]
for (const course_element of coursesArray) {
    ...
    text_p.textContent = `Course containes such modules as :`


    for (const elementLiElement of course_element.modules) {
        let element_li = document.createElement(`li`)
        element_li.textContent = elementLiElement

        modules_ul.appendChild(element_li)
    }
    ...
    block_course.appendChild(list)
    list.appendChild(text_p)
    list.appendChild(modules_ul)
